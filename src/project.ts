...
async function hygenDependencies(choices: ProjectChoices): Promise<void> {
  ...
  await execFunction(
    `hygen controller new --project ${choices.name} && hygen persistence new --project ${choices.name} && hygen app new --project ${choices.name} && hygen security apply --project ${choices.name}`
  );
}
